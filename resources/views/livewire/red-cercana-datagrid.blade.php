<div class="bg-gray-100 border rounded-lg border-gray-200 shadow-md">
    <div class="max-w-7xl mx-auto">
        ...
        <form wire:submit.prevent='leerDatosFormulario'>
            <div class="p-4 flex">
                ...
                <div class="flex flex-col w-full md:w-1/2">
                    <label class="block mb-1 text-sm text-gray-700 uppercase font-bold" for="termino">Buscar Red Cercana</label>
                    <input 
                        id="termino"
                        type="text"
                        placeholder="Buscar Red Cercana..."
            ...
            <table class="min-w-full table-auto bg-white border border-gray-200">
                <thead>
                    <tr class="bg-gray-200">
                        ...
                        <th class="px-4 py-2 text-center">Red Cercana</th>
                ...
                <tbody>
                    @foreach($redescercanas as $redcercana)
                        <tr class="border-t">
                            <td class="px-4 py-2 text-center">{{ $redcercana->id_red }}</td>
                            <td class="px-4 py-2 text-center">{{ $redcercana->descripcion }}</td>
                            <td class="px-4 py-2 text-center">

                                <button wire:click="openCreateModal({{ $redcercana->id_red }})" id="edit-btn-{{ $redcercana->id_red }}"
                                ...
                                <button wire:click="eliminarRedCercana({{ $redcercana->id_red }})" 
                                    onclick="confirm('¿Seguro de eliminar esta Red Cercana?') || event.stopImmediatePropagation()" id="delete-btn-{{ $redcercana->id_red}}"
        ...
        <!-- Paginación centrada -->
        <div class="my-10">
            {{ $redescercanas->links('pagination::tailwind') }}
        </div>
    </div>
        ...
        <div class="fixed left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 py-10">
            <div class="max-h-full w-full max-w-xl overflow-y-auto sm:rounded-2xl bg-white">
                <div class="w-full">
                    ...
                    <div class="mb-4">
                        <h1 class="font-extrabold">Agregar una Red Cercana</h1>
                    </div>
                    <form action="">
                        <div class="mb-4">
                            <label for="descripcion" class="block text-sm font-bold text-gray-700">Red Cercana</label>
                            <input autofocus wire:model="descripcion" type="text" id="descripcion" name="descripcion" 
                                    placeholder="Escriba una Nueva Red Cercana"
                    ...
                    <div class="mb-4 flex flex-row gap-6">
                        <button 
                                ...
                                wire:click="createorUpdateRedCercana">{{ isset($miRed->id_red) ? 'Actualizar' : 'Crear' }} Red Cercana</button>
                        <button 
