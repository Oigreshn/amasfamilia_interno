<div class="bg-gray-100 border rounded-lg border-gray-200 shadow-md">
    <div class="max-w-7xl mx-auto">
        ...
        <form wire:submit.prevent='leerDatosFormulario'>
            <div class="p-4 flex">
                ...
                <div class="flex flex-col w-full md:w-1/2">
                    <label class="block mb-1 text-sm text-gray-700 uppercase font-bold" for="termino">Buscar Permiso de Trabajo</label>
                    <input 
                        id="termino"
                        type="text"
                        placeholder="Buscar Permiso de Trabajo..."
            ...
            <table class="min-w-full table-auto bg-white border border-gray-200">
                <thead>
                    <tr class="bg-gray-200">
                        ...
                        <th class="px-4 py-2 text-center">Permiso de Trabajo</th>
                ...
                <tbody>
                    @foreach($permisostrabajos as $permisotrabajo)
                        <tr class="border-t">
                            <td class="px-4 py-2 text-center">{{ $permisotrabajo->id_permisotra }}</td>
                            <td class="px-4 py-2 text-center">{{ $permisotrabajo->descripcion }}</td>
                            <td class="px-4 py-2 text-center">

                                <button wire:click="openCreateModal({{ $permisotrabajo->id_permisotra }})" id="edit-btn-{{ $permisotrabajo->id_permisotra }}"
                                ...
                                <button wire:click="eliminarPermisoTrabajo({{ $permisotrabajo->id_permisotra }})" 
                                    onclick="confirm('¿Seguro de eliminar este Permiso de Trabajo?') || event.stopImmediatePropagation()" id="delete-btn-{{ $permisotrabajo->id_permisotra }}"
        ...
        <!-- Paginación centrada -->
        <div class="my-10">
            {{ $permisostrabajos->links('pagination::tailwind') }}
        </div>
    </div>
        ...
        <div class="fixed left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 py-10">
            <div class="max-h-full w-full max-w-xl overflow-y-auto sm:rounded-2xl bg-white">
                <div class="w-full">
                    ...
                    <div class="mb-4">
                        <h1 class="font-extrabold">Agregar un Permiso de Trabajo</h1>
                    </div>
                    <form action="">
                        <div class="mb-4">
                            <label for="descripcion" class="block text-sm font-bold text-gray-700">Permiso de Trabajo</label>
                            <input autofocus wire:model="descripcion" type="text" id="descripcion" name="descripcion" 
                                    placeholder="Escriba un Nuevo Permiso de Trabajo"
                    ...
                    <div class="mb-4 flex flex-row gap-6">
                        <button 
                                ...
                                wire:click="createorUpdatePermisoTrabajo">{{ isset($miPermisotra->id_permisotra) ? 'Actualizar' : 'Crear' }} Permiso Trabajo</button>
                        <button 
