<div class="bg-gray-100 border rounded-lg border-gray-200 shadow-md">
    <div class="max-w-7xl mx-auto">
        ...
        <form wire:submit.prevent='leerDatosFormulario'>
            <div class="p-4 flex">
                ...
                <div class="flex flex-col w-full md:w-1/2">
                    <label class="block mb-1 text-sm text-gray-700 uppercase font-bold" for="termino">Buscar Tipo de Tarjeta</label>
                    <input 
                        id="termino"
                        type="text"
            ...
            <table class="min-w-full table-auto bg-white border border-gray-200">
                <thead>
                    <tr class="bg-gray-200">
                        ...
                        <th class="px-4 py-2 text-center">Tipo de Tarjeta</th>
                ...
                <tbody>
                    @foreach($tipostarjetas as $tipotarjeta)
                        <tr class="border-t">
                            <td class="px-4 py-2 text-center">{{ $tipotarjeta->id_tipotarjeta }}</td>
                            <td class="px-4 py-2 text-center">{{ $tipotarjeta->descripcion }}</td>
                            <td class="px-4 py-2 text-center">

                                <button wire:click="openCreateModal({{ $tipotarjeta->id_tipotarjeta }})" id="edit-btn-{{ $tipotarjeta->id_tipotarjeta }}"
                                ...
                                <button wire:click="eliminarTipoTarjeta({{ $tipotarjeta->id_tipotarjeta }})" 
                                    onclick="confirm('¿Seguro de eliminar esta Tipo de Tarjeta?') || event.stopImmediatePropagation()" id="delete-btn-{{ $tipotarjeta->id_tipotarjeta }}"
        ...
        <!-- Paginación centrada -->
        <div class="my-10">
            {{ $tipostarjetas->links('pagination::tailwind') }}
        </div>
    </div>
        ...
        <div class="fixed left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 py-10">
            <div class="max-h-full w-full max-w-xl overflow-y-auto sm:rounded-2xl bg-white">
                <div class="w-full">
                    ...
                    <div class="mb-4">
                        <h1 class="font-extrabold">Agregar un Tipo de Tarjeta</h1>
                    </div>
                    <form action="">
                        <div class="mb-4">
                            <label for="descripcion" class="block text-sm font-bold text-gray-700">Tipo de Tarjeta</label>
                            <input autofocus wire:model="descripcion" type="text" id="descripcion" name="descripcion" 
                                    placeholder="Escriba un Tipo de Tarjeta"
                    ...
                    <div class="mb-4 flex flex-row gap-6">
                        <button 
                                ...
                                wire:click="createorUpdateTipoTarjeta">{{ isset($miTipoTarjeta->id_tipotarjeta) ? 'Actualizar' : 'Crear' }} Tipo de Tarjeta</button>
                        <button 
